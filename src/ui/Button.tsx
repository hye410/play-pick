...
const Button = (props: ButtonProps) => {
  const { size = "medium", color = "primary", children, onClick, disabled = false } = props;
  ...
  return (
    <button onClick={onClick} className={buttonVariants({ size, color: colorVariant() })} disabled={disabled}>
      {children}
    </button>
  );
};
